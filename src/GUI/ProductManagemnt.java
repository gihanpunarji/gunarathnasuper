...
public class ProductManagemnt extends javax.swing.JPanel {
    ...
    public void loadProductsTable(JTable table, List<Product> products) {
        try {
            ...
            ProductDAO dao = new ProductDAO();
            List<Product> productsList = (products != null) ? products : dao.getAllProducts();
            ...
            model.addTableModelListener(e -> {
                if (e.getType() == TableModelEvent.UPDATE) {
                    ...
                    try {
                        ...
                        String valueStr = table.getValueAt(row, col).toString().trim();
                        double newPrice = Double.parseDouble(valueStr);
                        ...
                        if (col == 4) {
                            success = dao.updateWeladapalaMila(id, newPrice);
                        } else if (col == 5) {
                            success = dao.updateApeMila(id, newPrice);
                        }
                        ...
                        JOptionPane.showMessageDialog(table, "⚠️ Invalid price! Enter a positive number.");
                        loadProductsTable(table, null); // revert
                    } catch (Exception ex2) {
                        ex2.printStackTrace();
